...
(function () {
    ...
    function initExtras() {
        ...
        // Escuchamos cuando se agrega un mensaje al DOM para ver si
        // necesitamos meter chips, formulario o botón de firma.
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mut) => {
                mut.addedNodes.forEach((node) => {
                    if (node.classList && node.classList.contains("chatbot-message--bot")) {
                        handleBotMessage(node);
                    }
                });
            });
        });
    ...
    async function startSession() {
        try {
            ...
            if (res && res.ok && res.data && res.data.session_id) {
                ...
                console.log("[chatbot_extras] Sesión real iniciada:", sessionState.sessionId);
            } else {
            ...
            console.warn("[chatbot_extras] Backend no disponible, activando MODO DEMO");
            showDemoBadge();
        }
    }
    ...
    async function sendUserMessage(text) {
        ...
        console.log(`[chatbot_extras] Respuesta en ${elapsed} ms (estado: ${sessionState.currentState})`);
        if (window.__chatbotMetrics) {
            window.__chatbotMetrics.push({ ms: elapsed, state: sessionState.currentState });
        }
    }
    ...
    async function submitDataForm(form, productCode) {
        ...
        try {
            ...
            response = { ok: false, error: { message: "Error de conexión al enviar." } };
        }
    ...
    async function demoReply(userText) {
        ...
        if (sessionState.currentState === "greeting" ||
            ...
            if (text.includes("soap") || text.includes("seguro")) {
                ...
                return {
                    ok: true,
                    data: {
                        reply: "El SOAP cubre accidentes personales con cobertura nacional. La prima depende del año del vehículo. ¿Te interesa contratarlo?",
                        state: "product_info",
            ...
            if (text.includes("depósito") || text.includes("deposito") ||
                ...
                return {
                    ok: true,
                    data: {
                        reply: "Nuestro Depósito a Plazo ofrece tasa del 0.45% mensual capitalizable. Plazos desde 30 hasta 365 días. ¿Quieres contratarlo?",
                        state: "product_info",
                        suggestions: ["Sí, contratar", "Más información"],
                    },
                };
            ...
            return {
                ok: true,
                data: {
                    reply: "Hola, soy el asistente de UMayor. ¿Te interesa SOAP o Depósito a Plazo?",
                    state: "discovery",
        ...
        if (sessionState.currentState === "product_info") {
            if (text.includes("contratar") || text.includes("sí") || text.includes("si")) {
                ...
                return {
                    ok: true,
                    data: {
                        reply: "Perfecto, necesito algunos datos para preparar el contrato.",
            ...
            data: {
                ...
                summary: {
                    ...
                    calculated: isSoap
                        ...
                        : {
                              interest: Math.round(payload.product_data.amount * 0.0045 *
    ...
    function getLastBotText() {
        const msgs = document.querySelectorAll(".chatbot-message--bot .chatbot-bubble");
        return msgs.length > 0 ? msgs[msgs.length - 1].textContent : "";
    }

    function detectProduct(replyText) {
        const t = (replyText || "").toLowerCase();
        if (t.includes("soap") || t.includes("vehículo") || t.includes("vehiculo")) return "soap";
