...
(function () {
  ...
  function initChatWidget() {
    ...
    toggleBtn.addEventListener("click", async () => {
      ...
      widget
        .querySelector(".cu-widget__panel")
    ...
    async function startSession() {
      try {
        // Siempre pedimos al backend que cree (o recupere) la sesión.
        // El backend es el único que sabe si el token aún es válido.
      ...
      try {
        ...
        products.slice(0, 4).forEach((p) => {
          ...
          chip.addEventListener("click", () =>
            sendMessage("Quiero información sobre " + p.name),
          );
    ...
    async function sendMessage(text) {
      ...
      try {
        ...
        if (result.error) {
          appendMessage("system", "Error: " + (result.error || "desconocido"));
        } else {
    ...
    function appendMessage(role, content) {
      ...
      const msg = el(`<div class="${cls}">${escapeHtml(content)}</div>`);
